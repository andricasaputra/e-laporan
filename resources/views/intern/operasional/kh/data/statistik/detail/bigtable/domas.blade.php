...
<h4 class="page-title">Detail Operasional Domestik Masuk Karantina Hewan</h4>
    ...
    <div class="row">
      <div class="col-md-12 card">
          ...
          <div class="card-header">
            Data Domestik Masuk Karantina Hewan Tahun <span id="yearSelect">{{ $tahun }}</span>
          </div>
          <div class="card-body">
             <table class="table table-responsive table-bordered w-100 d-block d-md-table" id="domaskh">
              <thead>
...
@section('script')
  ...
  <script>
    $(document).ready(function() {
      ...
      let container = $('#domaskh');

      datatablesOperasional(
        container, 
        '{{ route('api.kh.statistik.detail.bigtable.domas', [$tahun, $bulan, $userWilker === 1 ? null : $userWilker]) }}', 
        'kh'
      );
      ...
      $('#change_data').on('submit', function(e){
        ...
        if (year != '' && month == '' && wilker == '') {
          ...
          datatablesOperasional(container, 
            '{{ route('api.kh.statistik.detail.bigtable.domas') }}/' + year, 
          'kh');

        } else if(year != '' && month != '' && wilker == '') {

          datatablesOperasional(container, 
            '{{ route('api.kh.statistik.detail.bigtable.domas') }}/' + year + '/' + month, 
          'kh');

        } else {

          datatablesOperasional(container, 
            '{{ route('api.kh.statistik.detail.bigtable.domas') }}/' + year + '/' + month + '/' + wilker, 
          'kh');
