...
<h4 class="page-title">Detail Laporan Billing Karantina Hewan</h4>
    ...
    <div class="row">
      <div class="col-md-12 card">
          ...
          <div class="card-header">
            Data Laporan Billing Karantina Hewan Tahun <span id="yearSelect">{{ $tahun }}</span>
          </div>
          <div class="card-body">
             <table class="table table-responsive table-bordered w-100 d-block d-md-table" id="pnbpKh">
              <thead>
...
@section('script')
  ...
  <script>
    $(document).ready(function() {
      ...
      let container = $('#pnbpKh');

      dataTablesPnbpOperasional(
        container, 
        '{{ route('api.kh.statistik.detail.bigtable.pnbp', [$tahun, $bulan, $userWilker === 1 ? null : $userWilker]) }}'
      );
      ...
      $('#change_data').on('submit', function(e){
        ...
        if (year != '' && month == '' && wilker == '') {
          ...
          dataTablesPnbpOperasional(container, 
            '{{ route('api.kh.statistik.detail.bigtable.pnbp') }}/' + year);

        } else if(year != '' && month != '' && wilker == '') {

          dataTablesPnbpOperasional(container, 
            '{{ route('api.kh.statistik.detail.bigtable.pnbp') }}/' + year + '/' + month);

        } else {

          dataTablesPnbpOperasional(container, 
            '{{ route('api.kh.statistik.detail.bigtable.pnbp') }}/' + year + '/' + month + '/' + wilker);
