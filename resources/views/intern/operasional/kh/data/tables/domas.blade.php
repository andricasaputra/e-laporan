            ...
            <select class="form-control" name="year">
              @for($i = date('Y') - 3; $i < date('Y') + 2 ; $i++)
                ...
                @if($i == $tahun)
                  ...
                  <option value="{{ route('kh.view.page.domas', $i) }}" selected>{{ $i }}</option>

                @else

                  <option value="{{ route('kh.view.page.domas', $i) }}">{{ $i }}</option>
...
@section('script')
  ...
  <script>
    $(document).ready(function() {
	    ...
	    datatablesOperasional($('#domaskh'), '{{ route('api.kh.domas', $tahun) }}', 'kh');
