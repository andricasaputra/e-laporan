...
@section('title','Operasional - Laporan Billing')
...
<h4 class="page-title">Detail Laporan Billing Karantina Tumbuhan</h4>
<div class="d-flex align-items-center">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page">Detail Laporan Billing</li>
        </ol>
    </nav>
</div>
    ...
    <div class="row">
      <div class="col-md-12 card">
          ...
          <div class="card-header">
            Data Laporan Billing Karantina Tumbuhan Tahun <span id="yearSelect">{{ $tahun }}</span>
          </div>
          <div class="card-body">
             <table class="table table-responsive table-bordered w-100 d-block d-md-table" id="pnbpKt">
              <thead>
...
@section('script')
  ...
  <script>
    $(document).ready(function() {
      ...
      let container = $('#pnbpKt');

      dataTablesPnbpOperasional(
        container, 
        '{{ route('api.kt.statistik.detail.bigtable.pnbp', [$tahun, $bulan, $userWilker === 1 ? null : $userWilker]) }}'
      );
      ...
      $('#change_data').on('submit', function(e){
        ...
        if (year != '' && month == '' && wilker == '') {
          ...
          dataTablesPnbpOperasional(container, 
            '{{ route('api.kt.statistik.detail.bigtable.pnbp') }}/' + year);

        } else if(year != '' && month != '' && wilker == '') {

          dataTablesPnbpOperasional(container, 
            '{{ route('api.kt.statistik.detail.bigtable.pnbp') }}/' + year + '/' + month);

        } else {

          dataTablesPnbpOperasional(container, 
            '{{ route('api.kt.statistik.detail.bigtable.pnbp') }}/' + year + '/' + month + '/' + wilker);
