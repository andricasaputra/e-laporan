...
<div class="container-fluid">
  ...
  <div class="col">
    ...
    @if($datas['bulan'] !== null)
      <h3>
        Data Operasional Karantina Tumbuhan Bulan 
        {{ Tanggal::bulan($datas['bulan']) }} Tahun {{ $datas['tahun'] }}
        {{ $datas['wilker'] }}
      </h3>
    @else
      <h3>Data Operasional Karantina Tumbuhan Tahun {{ $datas['tahun'] }}</h3>
    @endif
  ...
  $(document).ready(function(){
    ...
    $('#change_data').on('submit', function(e){
      ...
      if (year != '' && month == '' && wilker == '') {
        ...
        window.location = '{{ route('show.operasional.kt') }}/' + year + '/' + month;

      } else {

        window.location = '{{ route('show.operasional.kt') }}/' + year + '/' + month + '/' + wilker;
